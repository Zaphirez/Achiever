...
Achiever.version = "1.2.0"
AchieverSettings.lastNotifiedVersion = nil
Achiever.playerName = UnitName("player")
Achiever.pendingChecks = {}
...
f:SetScript("OnEvent", function(_, _, prefix, msg, channel, sender)
    ...
    if respCheckID and theirVersion then
        ...
        if Achiever.pendingChecks[respCheckID] then
            ...
            DEFAULT_CHAT_FRAME:AddMessage("|cffaaff00Achiever:|r " .. sender .. " responded with version " .. theirVersion)
--[[         else
    ...
    if theirVersionPassive then
        ...
        if IsNewerVersion(theirVersionPassive, Achiever.version) and AchieverSettings.lastNotifiedVersion ~= theirVersionPassive then
            DEFAULT_CHAT_FRAME:AddMessage("|cffffaa00Achiever:|r New version available (v" ..
            theirVersionPassive .. ") from " .. sender)
            DEFAULT_CHAT_FRAME:AddMessage("|cffffaa00".."Achiever:" .. "|r" .. "You can download it from " .. "https://github.com/Zaphirez/Achiever/releases/latest")
